...
file.open("seir_dataset_sims.csv", ios::app);
int reg_idx = 0;
int threadID;
...
const int T = 10; // number of simulations per region

#pragma omp parallel for shared(file, allRegions, NUM_DAYS) \
private(reg_idx, dayCount, iterDiff, pastCheckpoint, N_INIT, STEP_TIME, NUM_STEPS, ITERATIONS_PER_DAY, region, threadID)
for(reg_idx = 0; reg_idx < allRegions.size(); reg_idx++) {

    threadID = omp_get_thread_num();
    region = allRegions[reg_idx];

    // Store original params for THIS region
    ...
    for(int sim = 0; sim <= T; sim++) {
        ...
        for(int i = 1; i <= ((NUM_DAYS/10)*(NUM_STEPS+1)); i++) {
            ...
            // Update shared map temporarily (needed for your computeSEIR)
            ...
            if(iterDiff >= ITERATIONS_PER_DAY) {
                ...
                #pragma omp critical
                {
                    file << sim << "," << region << "," << dayCount << ",";

                    file << local_params["A"] << ",";
                    file << local_params["alpha"] << ",";
                    file << local_params["delta"] << ",";
                    file << local_params["gamma"] << ",";
                    file << local_params["mew"] << ",";
                    file << local_params["mew1"] << ",";
                    file << local_params["N0"] << ",";
                    file << local_params["Vp"] << ",";
                    file << local_params["Vf"] << ",";

                    file << round(local_params["S0"]) << ",";
    ...
    #pragma omp critical
    {
        seir_params[region] = original_params;
    }
}	
